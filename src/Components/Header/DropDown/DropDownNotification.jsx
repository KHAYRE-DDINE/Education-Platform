        ...
        <MenuItems className="absolute right-[-60px] z-[9999] mt-3 w-84 origin-top-right rounded-2xl bg-white dark:bg-[#151c2c] border border-gray-100 dark:border-[#1e293b] shadow-2xl ring-1 ring-black/5 focus:outline-none overflow-hidden" style={{ width: "340px" }}>
          ...
          <div className="px-4 py-3 border-b border-gray-100 dark:border-[#1e293b] flex items-center justify-between">
            ...
            {unread > 0 && (
              <button
                onClick={markAllRead}
                className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium transition-colors"
              >
                  ...
                  {({ focus }) => (
                    <div
                      ...
                      className={`flex items-start gap-3 px-4 py-3 cursor-pointer transition-colors relative group ${
                        focus ? "bg-gray-50 dark:bg-[#1e293b]" : ""
                      } ${!n.read ? "bg-blue-50/40 dark:bg-blue-950/40" : "bg-white dark:bg-[#151c2c]"}`}
                    >
                      {/* Avatar or Icon */}
                      <div className="relative flex-shrink-0">
                        {n.avatar ? (
                          ...
                          <div className={`w-9 h-9 rounded-full ${n.iconBg} ${n.iconColor} dark:bg-slate-800 flex items-center justify-center`}>
                            {n.icon}
                          </div>
                        )}
                        {!n.read && (
                          <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-blue-500 border-2 border-white dark:border-[#151c2c] rounded-full" />
                        )}
                      </div>

                      <div className="flex-1 min-w-0">
                        <p className={`text-sm leading-snug ${!n.read ? "font-semibold text-gray-800 dark:text-white" : "font-medium text-gray-700 dark:text-gray-300"}`}>
                          {n.title}
                        </p>
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-all flex-shrink-0 mt-0.5"
                      >
                        <TbX size={14} />
                      </button>
                    </div>
          ...
          {/* Footer */}
          <div className="px-4 py-3 border-t border-gray-100 dark:border-[#1e293b] text-center">
